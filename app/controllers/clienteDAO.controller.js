...
exports.obtenerClientes = (req, res) => {
    Cliente.findAll( include["cliente_reservas"]).then(data => {
        ...
        res.status(500).send({
            mensaje: err.mensaje || "Ocurrio un error mientras se trata de obtener los clientes. Eso es todo lo que sabemos!"
        });
    });
};
...
exports.actualizarClienteByID = (req, res) => {
    ...
    Cliente.update(req.body, {
        ...
        res.status(500).send({
            mensaje: "Ocurrio un error mientras se trata de actualizar el cliente con id = " + id
        });
    });
};
...
exports.borrarClientes = (req, res) => {
    Cliente.destroy({
        ...
        res.status(500).send({
            mensaje: err.mensaje || "Algun error ocurrio mientras fueron removidos los cliente. Es todo lo que sabemos!"
        });
    });
};